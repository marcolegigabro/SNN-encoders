...
def rate_encode(pixels, T, p_max=0.8):
    ...
    each timestep with a rate proportional to the pixel's intensity.
    """
...
# ---------------------------------------------------------------------------
# Distortion: van Rossum (carried over from utils.py, included here so this
...
# ---------------------------------------------------------------------------
# A single SNN layer (no hidden layer -- same convention as AlphaTTFSLayer:
# the compressor and the decompressor are each ONE layer)
# ---------------------------------------------------------------------------
...
class SNNLayer(nn.Module):
    ...
    def forward(self, x):
        """x: (batch, T, d_in) -> spk: (batch, T, d_out). A neuron can spike
        several times over the T steps -- that is the key difference with
...
class SNNStack(nn.Module):
    """
    A stack of several SNNLayer modules -- unlike the TTFS case, nothing
    special is needed here: each SNNLayer consumes and produces a full
    spike train (batch, T, d), so they can simply be chained.
    """
    ...
    def forward(self, x):
        ...
        return x
# ---------------------------------------------------------------------------
# Delay channel: a discrete-time queue with bulk arrivals, shared by the N
# bottleneck neurons -- a discrete generalisation of Bedekar-Azizoglu /
# Anantharam-Verdu.
# ---------------------------------------------------------------------------

def _bulk_queue_delay_numpy(spikes_np, capacity):
    """spikes_np: (batch, T, N) 0/1 -> same shape, spikes delayed/dropped."""
    ...
    for b in range(batch):
        ...
        for t in range(T):
            ...
            for _ in range(n_depart):
                n = queue.pop(0)
                out[b, t, n] = 1.0
        # whatever remains in `queue` at the end of the T-step window is
        # lost (finite-capacity channel: a genuine channel with losses)
    return out


def bulk_queue_delay_channel(spikes, capacity=1):
    """
    Delay channel (shared queue, fixed capacity `capacity` spikes/timestep).
    NOT differentiable (discrete reassignment of spikes to new time indices)
    -- use it via `delay_channel` below, which adds the straight-through
    estimator.
    """
...
def delay_channel(spikes, capacity=1):
    """
    Trainable (straight-through) version of the delay channel: the forward
    pass uses the true, delayed/truncated spike train, the backward pass
    lets the loss gradient pass straight through as if the channel were
    transparent -- exactly the same principle already used for the
    neurons' own spike function via the straight-through estimator.
    """
